...
//    }
    ...
    public void validateLogin() {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                String role = resultSet.getString("Role"); // Récupérer le rôle de l'utilisateur

                loginMessageLabel.setText("Login successful!");

                if ("Admin".equals(role)) {
                    redirectToProduits(userId, userName); // Rediriger vers la page produits si l'utilisateur est Admin
                } else if ("User".equals(role)) {
                    redirectToUserDashboard(userId, userName); // Rediriger vers le tableau de bord utilisateur
                } else {
                    loginMessageLabel.setText("Access denied. Unknown role.");
                }
            } else {
    ...
    private void redirectToUserDashboard(int userId, String userName) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("userDashboard.fxml"));
            Parent root = loader.load();

            // Configure le contrôleur de la page userDashboard
            UserDashboardController userDashboardController = loader.getController();
            userDashboardController.setUserSession(userId, userName); // Passez UserID et Nom
            ...
            e.printStackTrace();
            loginMessageLabel.setText("Error loading user dashboard page. Check the file path.");
        }
    }
}